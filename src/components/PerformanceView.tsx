...
export const PerformanceView: React.FC<PerformanceViewProps> = ({
  ...
  return (
    <div className="min-h-screen bg-surface text-on-surface flex flex-col pb-28 sm:pb-24">
      {/* Top Header */}
      <header className="glass-header sticky top-0 z-20 px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between border-b border-outline-variant/40">
        <div className="flex items-center gap-2.5 sm:gap-4 overflow-hidden">
          {/* System Brand Logo Component */}
          <Logo onClick={() => onNavigate('landing')} size="sm" />

          <div className="h-6 w-px bg-outline-variant/40 hidden sm:block" />

          <div className="overflow-hidden">
            <div className="text-[10px] sm:text-xs text-primary font-bold">Final Candidate Report</div>
            <h1 className="text-xs sm:text-base font-bold font-display text-on-surface leading-tight truncate">
              {roleTitle} <span className="text-[11px] font-normal text-on-surface-variant hidden xs:inline">• {experienceLevel}</span>
            </h1>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          ...
          <button
            ...
            className="px-3 sm:px-4 py-2 rounded-xl bg-primary text-on-primary text-xs font-bold hover:bg-primary/90 transition-all shadow-md flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
          >
            {isGeneratingPDF ? (
              <>
          ...
          <button
            ...
            className="hidden md:flex px-3.5 py-2 rounded-xl bg-surface-container-high hover:bg-surface-container-highest text-on-surface text-xs font-bold transition-all items-center gap-1.5 cursor-pointer"
      ...
      {pdfSuccessMessage && (
        <div className="bg-secondary-container/40 border-b border-secondary/30 text-secondary px-4 sm:px-6 py-2.5 text-xs font-bold flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="material-symbols-outlined text-sm">check_circle</span>
            <span>Themed PDF Evaluation Report downloaded successfully!</span>
          </div>
          <button onClick={() => setPdfSuccessMessage(false)} className="text-secondary font-bold cursor-pointer">
            Dismiss
          </button>
        </div>
      )}
      ...
      <main className="max-w-7xl mx-auto w-full p-4 sm:p-6 lg:p-8 space-y-6 sm:space-y-8">
        ...
        <div className="p-5 sm:p-6 rounded-2xl bg-gradient-to-r from-primary/10 via-primary/5 to-transparent border border-primary/20 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3.5 w-full sm:w-auto">
            <div className="w-11 h-11 sm:w-12 sm:h-12 rounded-xl bg-primary text-on-primary flex items-center justify-center font-bold shadow-md shadow-primary/20 shrink-0">
          ...
          <div className="flex items-center gap-2.5 w-full sm:w-auto">
            <button
              onClick={handleDownloadPDF}
              disabled={isGeneratingPDF}
              className="w-full sm:w-auto px-5 py-2.5 rounded-xl bg-primary text-on-primary text-xs font-bold hover:bg-primary/90 transition-all shadow-md flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 shrink-0"
            >
              {isGeneratingPDF ? (
                <>
                  <span className="w-3.5 h-3.5 border-2 border-on-primary border-t-transparent rounded-full animate-spin" />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {/* Overall Score */}
          <div className="p-5 sm:p-6 rounded-2xl bg-surface-container-lowest border border-outline-variant/40 soft-shadow flex flex-col justify-between">
            <span className="text-xs font-bold uppercase tracking-wider text-on-surface-variant">Overall Performance</span>
            <div className="my-3 flex items-center gap-4">
              <div className="relative w-14 h-14 sm:w-16 sm:h-16 rounded-full circular-progress p-1.5 flex items-center justify-center shrink-0">
                <div className="w-full h-full bg-surface-container-lowest rounded-full flex items-center justify-center font-bold text-lg sm:text-xl text-primary font-display">
                  {report.overallScore}
                </div>
              </div>
              <div>
                <div className="text-xl sm:text-2xl font-bold font-display text-primary">{report.overallScore}%</div>
                <div className="text-xs text-on-surface-variant font-medium">Candidate Score</div>
              </div>
            </div>
          ...
          <div className="p-5 sm:p-6 rounded-2xl bg-surface-container-lowest border border-outline-variant/40 soft-shadow flex flex-col justify-between">
            ...
            <div className="my-2">
              <span className="inline-block px-3 py-1.5 rounded-xl bg-secondary-container/40 text-secondary font-bold text-xs sm:text-sm mb-2">
          ...
          <div className="p-5 sm:p-6 rounded-2xl bg-surface-container-lowest border border-outline-variant/40 soft-shadow flex flex-col justify-between">
            ...
            <div className="my-2">
              <div className="text-xl sm:text-2xl font-bold font-display text-on-surface mb-1">{report.confidence}</div>
          ...
          <div className="p-5 sm:p-6 rounded-2xl bg-surface-container-lowest border border-outline-variant/40 soft-shadow flex flex-col justify-between">
            ...
            <div className="my-2">
              <div className="text-2xl sm:text-3xl font-bold font-display text-on-surface mb-1">
        ...
        <div className="p-5 sm:p-8 rounded-2xl bg-surface-container-lowest border border-outline-variant/40 soft-shadow">
          ...
          <p className="text-xs sm:text-sm text-on-surface leading-relaxed bg-surface-container-low/50 p-3.5 sm:p-4 rounded-xl border border-outline-variant/20">
            {report.summary}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          ...
          {/* Topics to Improve */}
          <div className="p-5 sm:p-6 rounded-2xl bg-surface-container-lowest border border-outline-variant/40 soft-shadow sm:col-span-2 lg:col-span-1">
        ...
        {sessionItems.length > 0 && (
          <div className="p-5 sm:p-8 rounded-2xl bg-surface-container-lowest border border-outline-variant/40 soft-shadow">
            <h3 className="text-base sm:text-lg font-bold font-display text-on-surface mb-1">Question-by-Question Score Breakdown</h3>
            <p className="text-xs text-on-surface-variant mb-5 sm:mb-6">Score distribution out of 10 across all session questions</p>
            ...
            <div className="space-y-3 sm:space-y-4">
              {sessionItems.map((item, idx) => {
                ...
                return (
                  <div key={idx} className="p-3.5 sm:p-4 rounded-xl bg-surface-container-low border border-outline-variant/30">
                    <div className="flex items-center justify-between text-xs font-bold mb-2 gap-2">
                      ...
                      <span className="text-primary font-mono shrink-0">{qScore} / 10</span>
                    </div>
        ...
        <div className="p-5 sm:p-8 rounded-2xl bg-surface-container-lowest border border-outline-variant/40 soft-shadow">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
            <div>
              <h3 className="text-lg sm:text-xl font-bold font-display text-on-surface">Full Interview Transcript & AI Evaluation</h3>
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto">
              <button
                onClick={handleDownloadPDF}
                disabled={isGeneratingPDF}
                className="flex-1 sm:flex-initial px-3.5 sm:px-4 py-2 rounded-xl bg-primary text-on-primary text-xs font-bold hover:bg-primary/90 transition-all flex items-center justify-center gap-1.5 cursor-pointer disabled:opacity-50"
              >
                <span className="material-symbols-outlined text-sm">picture_as_pdf</span>
              ...
              <button
                onClick={handleDownloadReport}
                className="flex-1 sm:flex-initial px-3.5 sm:px-4 py-2 rounded-xl bg-surface-container-low hover:bg-surface-container-high text-xs font-bold text-on-surface flex items-center justify-center gap-1.5 cursor-pointer"
              >
          ...
          {sessionItems.length === 0 ? (
            <div className="p-6 sm:p-8 text-center text-xs text-on-surface-variant bg-surface-container-low rounded-xl border border-outline-variant/30">
              No active transcript items stored. Launch a practice session to generate a live transcript.
            </div>
          ) : (
            <div className="space-y-4 sm:space-y-6">
              {sessionItems.map((item, idx) => (
                <div key={idx} className="p-4 sm:p-6 rounded-2xl bg-surface-container-low/60 border border-outline-variant/30 space-y-3 sm:space-y-4">
                  ...
                  <div>
                    <h4 className="text-xs sm:text-sm font-bold text-on-surface mb-2 font-display">
                      "{item.question.question}"
                    </h4>
                    <div className="text-xs text-on-surface-variant italic bg-surface-container-lowest p-3 sm:p-3.5 rounded-xl border border-outline-variant/20 whitespace-pre-wrap">
      ...
      <div className="fixed bottom-0 inset-x-0 glass-header p-3 sm:p-4 border-t border-outline-variant/40 z-30 flex items-center justify-between max-w-7xl mx-auto rounded-t-2xl shadow-2xl gap-2">
        <div className="flex items-center gap-2">
          <button
            onClick={handleDownloadPDF}
            disabled={isGeneratingPDF}
            className="px-3.5 sm:px-5 py-2.5 rounded-xl bg-primary text-on-primary font-bold text-xs hover:bg-primary/90 transition-all shadow-md flex items-center gap-1.5 cursor-pointer disabled:opacity-50 shrink-0"
          >
            {isGeneratingPDF ? (
              <>
                <span className="w-3.5 h-3.5 border-2 border-on-primary border-t-transparent rounded-full animate-spin" />
          ...
          <button
            onClick={handleDownloadReport}
            className="hidden md:flex px-3.5 py-2.5 rounded-xl bg-surface-container-low text-on-surface hover:bg-surface-container-high transition-colors text-xs font-bold items-center gap-1.5 cursor-pointer"
          >
            <span className="material-symbols-outlined text-sm">description</span>
            <span>TXT</span>
          </button>
        ...
        <button
          ...
          className="px-4 sm:px-6 py-2.5 rounded-xl bg-surface-container-high hover:bg-surface-container-highest text-on-surface font-bold text-xs transition-all flex items-center gap-1.5 sm:gap-2 cursor-pointer truncate"
        >
